"""
S21 EMA Ribbon Bounce — Multi-Period Backtest Runner.

Runs S21 across 3 periods and 4 pairs:
...
from src.strategies_pkg.s21_ema_ribbon_bounce import S21_EMA_RibbonBounce
...
def main():
    ...
    print(f"{'='*110}")
    print("S21 EMA RIBBON BOUNCE — Multi-Period Backtest")
    ...
    for pair in PAIRS:
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S21_{pair}_training_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S21_{pair}_test_set_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S21_{pair}_2025_trades.csv"),
                              index=False)
    ...
    print(f"\n{'='*110}")
    print("SUMMARY — S21 EMA Ribbon Bounce")
    print(f"{'='*110}")
    ...
    # Save JSON
    out_path = os.path.join(RESULTS_DIR, "s21_backtest.json")
