"""
S20 Range Compression Breakout — Multi-Period Backtest Runner.

Runs S20 across 3 periods and 4 pairs:
...
from src.strategies_pkg.s20_range_compression import S20_RangeCompression
...
def load_test_set_data(pair):
    ...
    if not os.path.exists(proc_fp) or not os.path.exists(test_fp):
        print(f"    WARNING: Missing M15 data for {pair}")
        return None, None
...
def main():
    ...
    print(f"{'='*110}")
    print("S20 RANGE COMPRESSION BREAKOUT — Multi-Period Backtest")
    ...
    for pair in PAIRS:
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S20_{pair}_training_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S20_{pair}_test_set_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S20_{pair}_2025_trades.csv"),
                              index=False)
    ...
    print(f"\n{'='*110}")
    print("SUMMARY — S20 Range Compression Breakout")
    print(f"{'='*110}")
    ...
    # Save JSON
    out_path = os.path.join(RESULTS_DIR, "s20_backtest.json")
