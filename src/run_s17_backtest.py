"""
S17 Bollinger Band Rejection — Multi-Period Backtest Runner.

Runs S17 across 3 periods and 4 pairs:
...
from src.strategies_pkg.s17_bb_rejection import S17_BB_Rejection
...
def main():
    ...
    print(f"{'='*110}")
    print("S17 BOLLINGER BAND REJECTION — Multi-Period Backtest")
    ...
    for pair in PAIRS:
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S17_{pair}_training_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S17_{pair}_test_set_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S17_{pair}_2025_trades.csv"),
                              index=False)
    ...
    print(f"\n{'='*110}")
    print("SUMMARY — S17 Bollinger Band Rejection")
    print(f"{'='*110}")
    ...
    # Save JSON
    out_path = os.path.join(RESULTS_DIR, "s17_backtest.json")
