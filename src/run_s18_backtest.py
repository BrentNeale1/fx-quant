"""
S18 Failed Breakout Fade — Multi-Period Backtest Runner.

Runs S18 across 3 periods and 4 pairs:
...
from src.strategies_pkg.s18_failed_breakout import S18_FailedBreakout
...
def main():
    ...
    print(f"{'='*110}")
    print("S18 FAILED BREAKOUT FADE — Multi-Period Backtest")
    ...
    for pair in PAIRS:
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S18_{pair}_training_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S18_{pair}_test_set_trades.csv"),
                              index=False)
        ...
        if m15 is not None:
            ...
            if not trades.empty:
                trades.to_csv(os.path.join(RESULTS_DIR, f"S18_{pair}_2025_trades.csv"),
                              index=False)
    ...
    print(f"\n{'='*110}")
    print("SUMMARY — S18 Failed Breakout Fade")
    print(f"{'='*110}")
    ...
    # Save JSON
    out_path = os.path.join(RESULTS_DIR, "s18_backtest.json")
